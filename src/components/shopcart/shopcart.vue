<template>
  <div class="shopcart">
    ...
    <div class="ball-container">
      <transition name="drop" v-for="ball in balls" :key="ball.show" @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter">
    ...
    methods: {
      ...
      beforeEnter (el) { // 这个方法的执行是因为这是一个vue的监听事件
        ...
        while (count--) {
          ...
          if (ball.show) {
            let rect = ball.el.getBoundingClientRect() // 获取小球的相对于视口的位移
            let x = rect.left - 32
      ...
      enter (el, done) {
        ...
        this.$nextTick(() => { // 让动画异步执行，提高性能
          ...
          el.addEventListener('transitionend', done) // Vue为了知道过渡完成，必须设置相应的事件监听器
        })
      },
      afterEnter (el) {
